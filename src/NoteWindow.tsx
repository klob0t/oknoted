...
export default function NoteWindow({ id }: NoteWindowProps) {
  ...
  const spawnNewNote = () => {
    ...
    // Add to active notes
    const activeIds = JSON.parse(localStorage.getItem("active-note-ids") || "[]");
    localStorage.setItem("active-note-ids", JSON.stringify([...activeIds, newId]));
  ...
  const closeNote = async () => {
    ...
    const activeIds = JSON.parse(localStorage.getItem("active-note-ids") || "[]");
    localStorage.setItem("active-note-ids", JSON.stringify(activeIds.filter((aid: string) => aid !== id)));
  ...
  return (
    <div className="sticky-container" style={{ backgroundColor: currentColor.bg }}>
      ...
      <div data-tauri-drag-region className="drag-handle">
        <div data-tauri-drag-region className="color-dots">
          {COLORS.map((c, i) => (
            <div 
              key={c.name}
        ...
        <div data-tauri-drag-region className="controls-overlay">
          <button 
            className="icon-btn"
          ...
          >
            <svg viewBox="0 0 1000 1000" xmlns="http://www.w3.org/2000/svg">
              <path d="M544.45,192.12l114.96,100.12.11-.12,115.39,99.62-79.03,16.21-140.91,162.5-4.86,80.52-114.96-100.12.11-.12-115.39-99.62,79.03-16.21,140.69-162.25,4.86-80.52M544.44,102.12c-11.74,0-23.56,2.3-34.79,7-31.63,13.26-52.97,43.35-55.04,77.58l-3.02,50.05-100.69,116.12-49.12,10.08c-33.6,6.89-60.36,32.28-69,65.47-8.65,33.19,2.32,68.41,28.28,90.82l111.23,96.03c1.21,1.18,2.46,2.34,3.76,3.46l114.96,100.12c16.69,14.53,37.77,22.13,59.12,22.13,11.74,0,23.56-2.3,34.79-7,31.63-13.26,52.97-43.35,55.04-77.58l3.02-50.05,100.91-116.37,49.12-10.08c33.6-6.89,60.36-32.28,69.01-65.47,8.65-33.19-2.32-68.41-28.28-90.82l-115.39-99.62c-1.3-1.12-2.62-2.19-3.96-3.23l-110.82-96.51c-16.69-14.53-37.77-22.13-59.12-22.13h0Z" fill="rgba(0,0,0,0.4)"/>
              <polygon points="472.94 583.61 224.91 811 207.41 795.83 397.38 518.1 472.94 583.61 472.94 583.61" fill="rgba(0,0,0,0.4)"/>
            </svg>
          </button>
